...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const techName = slug.toUpperCase();
  return constructMetadata({
    title: `${techName} Case Studies & Projects | ANNEX`,
    description: `Explore our featured premium case studies and digital products built utilizing ${techName} technology.`,
    path: `/technologies/${slug}`,
  });
}

export default async function TechnologyTaxonomyPage({ params }: PageProps) {
  const { slug } = await params;
  const techFormatted = slug.toLowerCase().replace(/-/g, " ");
  ...
  const filteredProjects = projects.filter((p) => {
    ...
    return techs.some((t) => {
      ...
      return name ? name.toLowerCase() === techFormatted : false;
    });
  });
            ...
            <Stack gap={6} className="max-w-2xl">
              <Breadcrumbs
                items={[
                  ...
                  { name: `Technology: ${slug.toUpperCase()}`, href: `/technologies/${slug}` },
                ]}
              />
              ...
              <Heading level={1} className="text-4xl md:text-5xl font-black tracking-tightest mt-2">
                Built with {techFormatted.toUpperCase()}.
              </Heading>
              <p className="text-sm md:text-base text-muted leading-relaxed">
                Explore premium digital solutions, custom case studies, and scalable products crafted by ANNEX leveraging the power of {techFormatted}.
              </p>
            </Stack>
          </Container>
