...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const tag = await BlogRepository.getTagBySlug(slug);
  if (!tag) return constructMetadata({ title: "Tag Not Found", noIndex: true });
  return constructMetadata({
    title: `#${tag.name} Articles`,
    description: `Browse all ANNEX blog posts tagged with "${tag.name}".`,
...
export default async function TagPage({ params }: Props) {
  ...
  const [tag, posts] = await Promise.all([
    BlogRepository.getTagBySlug(slug),
    BlogRepository.getPostsByTag(slug),
  ]);
  if (!tag) notFound();
  ...
  return (
    <main className="min-h-screen">
      ...
      <section className="py-20 px-6 border-b border-border/10 bg-gradient-to-br from-primary/5 via-transparent to-transparent">
        <div className="max-w-5xl mx-auto">
          <nav className="text-xs font-mono text-muted flex items-center gap-2 mb-6">
            ...
            <ChevronRight className="w-3 h-3" />
            <span className="text-foreground/60">#{tag.name}</span>
          </nav>
          <p className="text-xs font-mono text-primary tracking-[0.3em] uppercase mb-3">Tag</p>
          <h1 className="text-4xl md:text-5xl font-black text-foreground mb-4">#{tag.name}</h1>
          <p className="text-sm text-muted/60 font-mono">{posts.length} articles</p>
        </div>
      </section>
      ...
      <div className="max-w-5xl mx-auto px-6 py-16">
        {posts.length === 0 ? (
          <p className="text-center text-muted font-mono py-16">No articles with this tag yet.</p>
        ) : (
